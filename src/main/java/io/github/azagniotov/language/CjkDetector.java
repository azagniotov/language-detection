...
class CjkDetector {
    ...
    static CjkDecision decide(final String input, final double threshold) {
        ...
        if (characterCounts.get(CharType.KATAKANA) == 0 && characterCounts.get(CharType.HIRAGANA) == 0) {
            if (characterCounts.get(CharType.JAPANESE_HAN) < characterCounts.get(CharType.CHINESE_HAN)) {
                ...
                final boolean decision = checkThreshold(
                        input, characterCounts.get(CharType.CHINESE_HAN), characterCounts.irrelevant(), threshold);
                return decision ? CjkDecision.DECISION_CHINESE : CjkDecision.DECISION_NONE;
            } else {
                // We have a Japanese input
                final boolean decision =
                        checkThreshold(input, characterCounts.allJapanese(), characterCounts.irrelevant(), threshold);
    ...
    private static boolean checkThreshold(
            final String input, final double totalMatchedChars, final double irrelevantChars, final double threshold) {
