...
public class LanguageDetectorTest {
    ...
    static {
        final LanguageDetectorFactory factory = new LanguageDetectorFactory(DEFAULT_SETTINGS_ALL_LANGUAGES);
        DEFAULT_DETECTOR =
    ...
    public void setUp() throws Exception {
        ...
        final InputStream enTest = new ByteArrayInputStream(
        ...
        final InputStream frTest = new ByteArrayInputStream(
        ...
        final InputStream jaTest = new ByteArrayInputStream(
    ...
    public void shouldDetectJapaneseDataset() throws Exception {
        assertEquals(
                languageDetector.detectAll("\u3042\u3042\u3042\u3042a").get(0).getIsoCode639_1(), "ja_test");
    }
    ...
    public final void languageDetectorShouldDetectChinese() throws Exception {
        ...
        assertEquals(
                "zh-cn",
                detector.detectAll("位于美国首都华盛顿都会圈的希望中文学校５日晚举办活动庆祝建立２０周年。"
    ...
    @Test
    public final void languageDetectorShouldDetectShortStringsWithDefaultSanitization() throws Exception {
        ...
        assertEquals(
                "ja", DEFAULT_DETECTOR.detectAll("\"ヨキ\" AND \"杉山\"").get(0).getIsoCode639_1());
        assertEquals(
                "ja",
                DEFAULT_DETECTOR.detectAll("\"ヨキ\" AND \"杉山\" OR \"分布\"").get(0).getIsoCode639_1());
        assertEquals(
                "ja", DEFAULT_DETECTOR.detectAll("#4_pj_23D002_HCMJ_デジ戦").get(0).getIsoCode639_1());
        assertEquals(
                "ja",
                DEFAULT_DETECTOR
                        .detectAll("in1729x01_J-LCM刷新プロジェクト.docx")
                        .get(0)
                        .getIsoCode639_1());
        ...
        assertEquals(
                "ja",
                DEFAULT_DETECTOR
                        .detectAll("ﾊｰﾄﾞｶﾌﾟｾﾙ(ｻｲｽﾞ3号 NATURAL B／C)")
                        .get(0)
                        .getIsoCode639_1());
        assertEquals("ja", DEFAULT_DETECTOR.detectAll("Microsoft　インポート").get(0).getIsoCode639_1());

        assertEquals(
                "da", DEFAULT_DETECTOR.detectAll("Ｃｕｌｔｕｒｅ　ｏｆ　Ｊａｐａｎ.pdf").get(0).getIsoCode639_1());

        assertEquals(
                "ca",
                DEFAULT_DETECTOR
                        .detectAll("ﾊｰﾄﾞｶﾌﾟｾﾙ(ｻｲｽﾞ3号 NATURAL B／C).pdf")
                        .get(0)
                        .getIsoCode639_1());
        assertEquals(
                "ca", DEFAULT_DETECTOR.detectAll("刷新eclipseRCPExt.pdf").get(0).getIsoCode639_1());
        assertEquals("ca", DEFAULT_DETECTOR.detectAll("report.xls").get(0).getIsoCode639_1());

        assertEquals(
                "en",
                DEFAULT_DETECTOR.detectAll("This is a very small test").get(0).getIsoCode639_1());
        ...
        assertEquals(
                "zh-tw", DEFAULT_DETECTOR.detectAll("１２３４５６７８９０.xls 報告").get(0).getIsoCode639_1());
        assertEquals("zh-tw", DEFAULT_DETECTOR.detectAll("富山県高岡市.txt").get(0).getIsoCode639_1());
        assertEquals(
                "zh-tw",
                DEFAULT_DETECTOR
                        .detectAll("【12.21s】【流山運動公園：流山市市野谷】【中村P：3台】")
                        .get(0)
                        .getIsoCode639_1());
        ...
        assertEquals(
                "zh-tw",
                DEFAULT_DETECTOR.detectAll("AND \"杉山\" OR \"分布\"").get(0).getIsoCode639_1());

        assertEquals("zh-cn", DEFAULT_DETECTOR.detectAll("㈱_(株)_①②③_㈱㈲㈹").get(0).getIsoCode639_1());
        assertEquals(
                "zh-cn",
                DEFAULT_DETECTOR.detectAll("帮助他们以截然不同的方式探索和分析数据.pdf").get(0).getIsoCode639_1());
    }
